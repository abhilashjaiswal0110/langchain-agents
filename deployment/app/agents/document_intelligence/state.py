...
class DocumentIntelligenceState(BaseModel):
    ...
    documents: list[dict[str, Any]] = Field(
        default_factory=list,
        description="Loaded documents with metadata (id, filename, type, chunks, language)"
    )
    active_document_ids: list[str] = Field(
        default_factory=list,
        description="Currently active document IDs for querying"
    )
    ...
    retrieved_context: list[dict[str, Any]] = Field(
        default_factory=list,
        description="Retrieved document chunks with source info"
    )
    ...
    web_search_results: list[dict[str, Any]] = Field(
        default_factory=list,
        description="Results from web searches"
    )
