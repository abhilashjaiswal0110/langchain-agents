...
class ITSupportState(BaseModel):
    ...
    priority: Literal["low", "medium", "high", "critical"] = Field(default="medium", description="Ticket priority")
    ...
    status: Literal["new", "in_progress", "pending_approval", "escalated", "resolved"] = Field(
        default="new", description="Ticket status"
    )
...
@tool
@tool_error_handler
def create_ticket(description: str, category: str, priority: str = "medium", user_email: str = "") -> str:
...
def check_action_approval(action: str) -> str:
    ...
    """
    requires_approval = any(sensitive in action.lower() for sensitive in SENSITIVE_ACTIONS)
...
Always prioritize security over convenience."""
    ...
    def _build_graph(self) -> StateGraph:
            ...
            approval = interrupt(
                {
                    ...
                    "prompt": "Please approve or deny this action. Reply 'approve' or 'deny' with optional notes.",
                }
            )
        ...
        graph.add_conditional_edges("triage", should_continue, {"tools": "tools", "approval": "approval", "end": END})
        graph.add_edge("tools", "triage")
        graph.add_conditional_edges("approval", check_approval_needed, {"approval": "approval", "continue": "triage"})
