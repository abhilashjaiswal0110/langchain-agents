...
def scan_security_issues(
    ...
    for i, line in enumerate(lines, 1):
        ...
        # Command Injection
        if re.search(
            r'(os\.system|subprocess\.call|subprocess\.run|exec|eval)\s*\([^)]*(\+|%|f["\']|\.format)', line, re.I
        ):
            issues.append(
                {
                    "id": f"{scan_id}-{len(issues) + 1}",
...
def detect_secrets(
    ...
    for i, line in enumerate(lines, 1):
        for secret_type, (pattern, severity) in patterns.items():
            ...
            if matches or re.search(pattern, line, re.I):
                ...
                masked = re.sub(
                    pattern,
                    lambda m: m.group(0)[:10] + "..." + m.group(0)[-4:] if len(m.group(0)) > 14 else "****",
                    line,
                )
    ...
    result = {
        ...
        if secrets
        else ["No secrets detected - continue following best practices"],
    }
...
def analyze_dependencies_security(
    ...
    result = {
        ...
        else ["All dependencies appear secure"],
    }
